var projects = {
	product_page_redesign: {
		url: 'iinet-plan-pages.php',
		title: 'Product Page Redesign',
		client: 'iiNet',
		brand_colours: [ '#ff8200', '#d21f2a', '#0090ab', '#808080', '#fff' ],
		skills: ['HTML5', 'CSS3', 'PHP', 'SASS', 'AngularJS', 'UX Design', 'User Testing', 'OOCSS & BEM'],
		year: '2016',
		website: 'www.iinet.net.au',
		website_url: '//www.iinet.net.au/internet-products/broadband/naked-dsl',
		thumbnail: 'iinet-plan-pages-thumb.png'
	},
	mobii_manager: {
		url: 'mobii-manager.php',
		title: 'Mobile App Design',
		client: 'iiNet',
		brand_colours: [ '#F78F1D', '#BD1234', '#F5F5F5', '#32373B', '#019EB1' ],
		skills: ['mobile app design', 'UI/UX', 'interaction design', 'Adobe Photoshop', 'rapid prototype'],
		year: '2014',
		//website: '',
		//website_url: '',
		thumbnail: 'mobii-manager-thumb.png'
	},
	multicomm: {
		url: 'multicomm.php',
		title: 'Multicomm Website',
		client: 'Multicomm',
		brand_colours: [ '#FF9725', '#FFBB25', '#0C2D6C', '#3193FD', '#F7F4F3' ],
		skills: ['HTML5', 'CSS3', 'jQuery', 'branding', 'PHP', 'UX design', 'logo design', 'interaction design', 'freelance'],
		year: '2015',
		website: 'www.multicomm.com.au',
		website_url: '//www.multicomm.com.au/',
		thumbnail: 'multicomm-thumb.png'
	},
	bankwest: {
		url: 'bankwest.php',
		title: 'Bankwest Collateral',
		client: 'Bankwest',
		brand_colours: [ '#ff8200', '#d21f2a', '#0090ab', '#808080', '#fff' ],
		skills: ['HTML5', 'CSS3', 'PHP', 'AngularJS', 'UX Design', 'User Testing', 'SASS', 'OOCSS/BEM'],
		//year: '2012',
		website: 'www.iinet.net.au',
		website_url: '//www.iinet.net.au/internet-products/broadband/naked-dsl',
		thumbnail: 'bankwest-thumb.png'		
	}
}; 

var projectOrder = ["product_page_redesign", "mobii_manager", "multicomm"];//, "bankwest"]; 